...
void ClimateModule::tick(uint32_t nowMs)
{
    ...
    if (m_state.manualMode) return;

    if (m_state.safeMode || !m_state.sessionActive) {
        allOff();
        return;
    }
...
void ClimateModule::controlTemp(uint32_t nowMs)
{
    if (!m_state.tempSensorOk) {
        if (m_heater.isOn() && canSwitch(nowMs, m_lastHeaterSwitchMs)) m_heater.off();
...
void ClimateModule::controlHumidity(uint32_t nowMs)
{
    if (!m_state.humiSensorOk) {
        if (m_humidifier.isOn() && canSwitch(nowMs, m_lastHumidifierSwitchMs)) m_humidifier.off();
...
void ClimateModule::checkAlarms(uint32_t delta)
{
    ...
    if (!m_state.tempAlarmActive && !m_state.humiAlarmActive) m_buzzer.off();
}
